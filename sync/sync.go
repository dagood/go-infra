...
func MakeBranchPRs(f *Flags, dir string, entry *ConfigEntry) ([]SyncResult, error) {
	...
	if *f.CreateBranches {
		for _, b := range branches {
			check := newGitCmd("ls-remote", "--exit-code", "--heads", entry.Target, b.Name)
			if err := run(check); err != nil {
				...
				if *f.DryRun {
					return nil, errors.New("would have pushed a new branch to the target repository to kick off a new version, but this is a dry run. Cannot continue")
				}
			}
		}
	}
